...
export default function ExportarEmails() {
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost/API/estatisticaEmail.php?${params}`);
      const resData = await response.json();

      if (!response.ok) {
        throw new Error(resData.erro || 'Erro ao carregar estatísticas de emails');
      }
  ...
  // Monta o texto da célula superior esquerda (Ex: "13-04-2023 a 18-05-2026 Serviço")
  ...
  const exportarExcel = () => {
    ...
    // 3. Criar e configurar o ficheiro Excel
    const ws = XLSX.utils.aoa_to_sheet(matriz);
    const wb = XLSX.utils.book_new();
    
    // Configura a largura das colunas (A primeira larga, as restantes controladas)
    const larguras = [{ wch: 50 }]; 
    categorias.forEach(() => {
      larguras.push({ wch: 18 });   
    });
    ws['!cols'] = larguras;

    XLSX.utils.book_append_sheet(wb, ws, "Emails");
    XLSX.writeFile(wb, `Estatistica_Emails_${filtros.unidade || 'Geral'}.xlsx`);
  };
  ...
  const exportarPDF = async () => {
    ...
    try {
      ...
      const pdf = new jsPDF('l', 'mm', 'a4'); // 'l' = Horizontal/Landscape para tabelas largas
      const pdfWidth = pdf.internal.pageSize.getWidth();
      const pdfHeight = (canvas.height * pdfWidth) / canvas.width;

      pdf.addImage(imgData, 'PNG', 0, 10, pdfWidth, pdfHeight);
      pdf.save(`Estatistica_Emails_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (error) {
    ...
    <div className="page-wrapper">
      ...
      <main className="main-content list-page">
        <div className="container-1200 stats-page-container">
          ...
          {/* Tabela Grelha idêntica à imagem */}
                ...
                {listaDados.length > 0 ? listaDados.map((linha, idx) => (
                  ...
                  <tr>
                    <td colSpan={categorias.length + 1} style={{ textAlign: 'center', padding: '20px', border: '1px solid #000' }}>
                      Nenhum registo de email encontrado.
                    </td>
                  </tr>
                )}
              </tbody>
